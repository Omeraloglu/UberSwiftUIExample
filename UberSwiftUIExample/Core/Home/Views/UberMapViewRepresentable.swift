    ...
    func updateUIView(_ uiView: UIViewType, context: Context) {
        ...
        case .locationSelected:
            if let coordinate = locationViewModel.selectedUberLocation?.coordinate {
                ...
                context.coordinator.configurrePolyline(withDestinationCoordinate: coordinate)
...
extension UberMapViewRepresentable {
    ...
    class MapCoordinator: NSObject, MKMapViewDelegate {
        ...
        func configurrePolyline(withDestinationCoordinate coodinate: CLLocationCoordinate2D) {
            guard let userLocationCoordinate = self.userLocationCoordinate else { return }
            parent.locationViewModel.getDestinationRoute(from: userLocationCoordinate,
                                                         to: coodinate) { route in
